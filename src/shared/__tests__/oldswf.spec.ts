...
describe('extractSwfPath', () => {
  ...
  it('实测的四种目录写法都能取到，且不依赖文件名等于游戏 ID', () => {
    // 202914 → game.swf；100 → 13278.swf；50000 → 4632.swf；109087 → 旧正则唯一认得的一种
    ...
    expect(extractSwfPath(page('/data/game_2024/109087.swf'))).toBe('/data/game_2024/109087.swf');
  });

  it('容忍绝对地址与查询串，只保留 pathname', () => {
    expect(extractSwfPath(page('https://oldswf.com/data/game/9.swf?x=1'))).toBe('/data/game/9.swf');
  });
...
describe('parseFilesXml', () => {
  it('解析实测清单原文（224992 的 files.xml）', () => {
    expect(parseFilesXml('<files>\n\t<item>game.swf</item>\n</files>')).toEqual(['game.swf']);
  });

  it('保留多条目顺序，过滤非 SWF 与重复项', () => {
    expect(
      parseFilesXml(
        '<files><item>main.SWF</item><item>bg.mp3</item><item>main.SWF</item><item>b.swf</item></files>'
      )
    ).toEqual(['main.SWF', 'b.swf']);
  });

  it('空文档或无 item 时返回空数组', () => {
    expect(parseFilesXml('')).toEqual([]);
    expect(parseFilesXml('<files></files>')).toEqual([]);
  });
});
